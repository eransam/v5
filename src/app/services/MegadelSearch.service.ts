...
export class MegadelSearchService {
  ...
  async get_data_from_close_month_by_grower_meforat(
    ...
    chosenMonthControl_excel: any,
    to_chosenMonthControl_excel: any
  ): Promise<any[]> {
    const item = await firstValueFrom(
      this.http.get<any[]>(
        `${environment.apiPath}growerService.asmx/get_data_from_close_month_by_grower_meforat?chosenYearControl_excel=${chosenYearControl_excel}&to_chosenYearControl_excel=${to_chosenYearControl_excel}&chosenMonthControl_excel=${chosenMonthControl_excel}&to_chosenMonthControl_excel=${to_chosenMonthControl_excel}`
      )
    );
    return item;
  }

  async get_data_from_close_month_by_msvk_merokaz(
    chosenYearControl_excel: any,
    to_chosenYearControl_excel: any,
    chosenMonthControl_excel: any,
    to_chosenMonthControl_excel: any
    ...
    const item = await firstValueFrom(
      this.http.get<any[]>(
        `${environment.apiPath}growerService.asmx/get_data_from_close_month_by_msvk_merokaz?chosenYearControl_excel=${chosenYearControl_excel}&to_chosenYearControl_excel=${to_chosenYearControl_excel}&chosenMonthControl_excel=${chosenMonthControl_excel}&to_chosenMonthControl_excel=${to_chosenMonthControl_excel}`
      )
    );
    return item;
  }

  async get_data_growers_from_close_month(
    chosenYearControl_excel,
    to_chosenYearControl_excel,
    chosenMonthControl_excel,
    to_chosenMonthControl_excel,
    chosenShlohaControl_excel,
    usernameControl_test,
    paymentControl_excel,
    siteNumControl_test,
    gidulHotzNumControl_test,
    msvk_name_Control_test,
    msvk_code_Control_test,
  ...
  async GET_prices_and_dates_with_updates(
    mh_sug_mhir: any,
    mh_shana: any,
    mh_tzrt: any
  ): Promise<any[]> {
    const item = await firstValueFrom(
      this.http.get<any[]>(
        `${environment.apiPath}growerService.asmx/GET_prices_and_dates_with_updates?mh_sug_mhir=${mh_sug_mhir}&mh_shana=${mh_shana}&mh_tzrt=${mh_tzrt}`
      )
    );
    return item;
  }

  async GET_all_current_prices_and_dates(
    mh_sug_mhir: any,
    mh_shana: any,
    mh_tzrt: any
  ): Promise<any[]> {
    const item = await firstValueFrom(
      this.http.get<any[]>(
        `${environment.apiPath}growerService.asmx/GET_all_current_prices_and_dates?mh_sug_mhir=${mh_sug_mhir}&mh_shana=${mh_shana}&mh_tzrt=${mh_tzrt}`
      )
    );
    return item;
  }

  async GET_all_prices_and_dates_new_2(
    mh_sug_mhir: any,
    mh_shana: any
  ): Promise<any[]> {
    const item = await firstValueFrom(
